...
import com.example.ict_services_realm.repository.AuthRepository
import com.example.ict_services_realm.repository.RealmAuthRepository
...
sealed class LoginEvent(val severity: EventSeverity, val message: String) {
    class GoToLanding(severity: EventSeverity, message: String): LoginEvent(severity, message)
    ...
    fun login(email: String, password: String, fromCreation: Boolean = false) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            runCatching {
                ...
                _event.emit(LoginEvent.GoToLanding(EventSeverity.INFO, "User logged in successfully."))
            }.onFailure { ex: Throwable ->
                _state.value = state.value.copy(enabled = true)
                val message = when (ex) {
